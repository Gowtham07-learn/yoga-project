// poseyoga prototype app.js
// Requires MediaPipe scripts included in index.html

// ----- Levels (reference images + guessed reference angles) -----
// Angles are approximate guesses (in degrees) for: LShoulder, RShoulder, LElbow, RElbow, LHip, RHip, LKnee, RKnee
const LEVELS = [
  {
    id: 1,
...
function computeSimilarityMap(detectedAngles, levelRef){
  ...
  for(const key in levelRef.angles){
    ...
    if(typeof val === "number" && !isNaN(val)){
      ...
      const s = Math.max(0, 1 - (d / tol)); // 0..1
...
// ---- drawing helpers using MediaPipe drawing_utils when available ----
function drawPoseOnCanvas(landmarks){
  // Fit overlay to video
  ...
  // Use MediaPipe drawing utils if available
  try {
  ...
  // fallback simple points & lines (scaled)
  ctx.fillStyle = "#0a6";
...
function onResults(results){
  ...
  if(activeRun){
    ...
    // show a live estimate even when not running
...
// run logic: 3s countdown then 30s sampling
...
// initialize simple sizing of canvas when video metadata is available
videoElement.addEventListener('loadedmetadata', ()=>{
  overlay.width = videoElement.videoWidth;
  overlay.height = videoElement.videoHeight;
});

// auto-setup pose library in the background (does not start camera)
setupPose().catch(()=>{ /* ignore but loaded when camera starts */ });
